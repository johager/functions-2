...
const postTaxPrices = prices.map(element => element += (element * .07));
...
const totalPopulation = populations.reduce((acc, ele) => acc + ele)
...
const totals = orders.map((order) => order.price * (order.tax + 1))
...
const bobTotal = purchases.filter(purch => purch.owner === `Bob`).map((purch) => purch.price).reduce((acc, ele) => acc + ele)
console.log("bobTotal:", bobTotal)